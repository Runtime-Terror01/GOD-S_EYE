...
# Assuming data_types are defined in a sibling file
from .data_types import REPORT, ALERTS, FRAMES_BATCH, FRAME_META_BATCH
...
class VLMAnalysisNode(BaseNode):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                # MODIFIED: Changed input from 'video' to 'frames' and 'frame_metadata'
                "frames": ("FRAMES_BATCH",),
    ...
    def analyze(self, frames, frame_metadata, api_key: str, execute: bool, 
        ...
        return self.process(frames, frame_metadata, storage_path, custom_query, custom_instruction)
    
    def process(self, frames: FRAMES_BATCH, frame_metadata: FRAME_META_BATCH, storage_path: str, 
                custom_query: str = "", custom_instruction: str = "") -> Tuple[REPORT, ALERTS]:
        """
        Process frames by creating a temporary video, running VLM analysis, and generating a report.
        """
        if not frames or not frame_metadata:
            print("[VLM] Warning: Received empty frames or metadata. Skipping analysis.")
            return (None, [])

        # Initialize Gemini API
        genai.configure(api_key=self.api_key)
        
        # Use session_id from incoming metadata
        self.session_id = frame_metadata[0].get("session_id", str(uuid.uuid4())[:8])

        temp_video_path = None
        try:
            # NEW: Create a temporary video file from the frames
            temp_video_path = self._create_temp_video_from_frames(frames, frame_metadata)
            
            print(f"[VLM] Processing temporary video: {temp_video_path}")
            print(f"[VLM] Session ID: {self.session_id}")
            
            # Analyze the newly created video with Gemini
            ...
            report = {
                ...
                "video_path": f"Generated from frames for session {self.session_id}" # Path is now conceptual
            }
            ...
            # Save report
            report_path = metadata_dir / "vlm_report.json"
            with open(report_path, 'w') as f:
                json.dump(report, f, indent=2)
            
            # Save alerts
            if alerts:
                alerts_path = metadata_dir / "vlm_alerts.json"
            ...
            # NEW: Critical cleanup step to remove the temporary video file
            if temp_video_path and os.path.exists(temp_video_path):
                try:
                    os.remove(temp_video_path)
                    print(f"[VLM] ✅ Cleaned up temporary video: {temp_video_path}")
                except Exception as e:
                    print(f"[VLM] ⚠️ Error cleaning up temporary file {temp_video_path}: {e}")
    
    def _create_temp_video_from_frames(self, frames: List[np.ndarray], metadata: List[Dict[str, Any]]) -> str:
        """
        NEW: Creates a temporary MP4 video from a list of frames (NumPy arrays).
        """
        if not frames:
            raise ValueError("Cannot create video from an empty list of frames.")

        # Get video properties from the first frame and metadata
        height, width, layers = frames[0].shape
        
        # Calculate FPS from timestamps for accuracy
        fps = 30.0  # Default fallback
        if len(metadata) > 1:
            time_diff = metadata[1]['timestamp_sec'] - metadata[0]['timestamp_sec']
            if time_diff > 0:
                fps = 1.0 / time_diff
        
        print(f"[VLM] Reconstructing video: {width}x{height} @ {fps:.2f} FPS")

        # Create a temporary file path
        temp_dir = tempfile.gettempdir()
        temp_path = os.path.join(temp_dir, f"temp_vlm_analysis_{self.session_id}.mp4")
        ...
        if not out.isOpened():
            raise IOError("Could not open video writer. Check if OpenCV is correctly installed with ffmpeg support.")
        ...
        print(f"[VLM] Created temporary video at: {temp_path}")
        return temp_path

    # The problematic _get_video_path method is no longer needed for the main workflow
    # but is kept here in case it's useful for other purposes.
    ...
    def _analyze_video_with_gemini(self, video_path: str, query: str, instruction: str) -> Tuple[str, str]:
        ...
        # Cleanup the uploaded file on Google's side
        try:
            genai.delete_file(uploaded_file.name)
        except Exception as e:
            print(f"[VLM] Warning: Could not delete uploaded file from Gemini: {e}")
...
class SemanticSearchNode(BaseNode):
    ...
    def search(self, query: str, storage_path: str, max_results: int = 5):
        ...
        return ("\n".join(formatted),)
    # ... The rest of the class methods (_parse_model_response, _parse_summary, etc.) remain unchanged ...
    # ... (Please copy them from your original file to here) ...
    # <--- PASTE THE REST OF THE METHODS FROM YOUR ORIGINAL FILE HERE --->
    # The methods from _parse_model_response onwards are identical.

# The SemanticSearchNode class also remains unchanged.
# <--- PASTE THE SemanticSearchNode CLASS HERE --->